...
fun main() {
    do {
        ...
        do {
            ...
            val (strike, ball) = countStrikeBall(answerNumList, inputNumList)
...
// 스트라이크, 볼 카운트
fun countStrikeBall(randomList: List<Int>, numList: List<Int>): Pair<Int, Int> {
    var strike: Int = 0
    var ball: Int = 0
    for (i in 0 until 3) {
        if (numList[i] in randomList) {
            if (numList[i] == randomList[i]) strike += 1
            else ball += 1
        }
    }
...
fun inputNumberException(numList: List<Int>) {
    ...
    when {
        (numList.size != 3) ->
            if (numList.isEmpty()) throw IllegalArgumentException("잘못된 입력: 입력된 값이 없습니다.")
            else throw IllegalArgumentException("잘못된 입력: 3개의 숫자가 아닙니다.")
